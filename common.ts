...
import { readableStreamToString, UniCourtError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new UniCourtError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new UniCourtError(e, e.response?.data)
                }
                if (innerError instanceof UniCourtError) {
                    // Got "UniCourtError" from the above try block
                    throw innerError;
                }
